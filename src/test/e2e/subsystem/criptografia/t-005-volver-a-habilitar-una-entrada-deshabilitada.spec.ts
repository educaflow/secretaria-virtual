...
// origen: ESC-005  |  verifica: —
// fuente: .sdd/drafts/2026-08-10_23-21_deshabilitar-certificado-digital/test-e2e-desc/t-005-volver-a-habilitar-una-entrada-deshabilitada.desc.md
...
test.describe('Certificados digitales', () => {
  test('Volver a habilitar una entrada deshabilitada', async ({ page }) => {
    ...
    try {
      ...
      await filaDelDni(page).first().click();
      const habilitadoAlta = casillaHabilitado(page);
      await expect(habilitadoAlta).toBeVisible();
      await expect(habilitadoAlta).toBeChecked();
      await habilitadoAlta.uncheck();
      await expect(habilitadoAlta).not.toBeChecked();
      await guardarYVolverAlListado(page);
      ...
      // Paso 6: Cuando abre de nuevo la fila del DNI «85432016B».
      await filaDelDni(page).first().click();
      const habilitado = casillaHabilitado(page);
      await expect(habilitado).toBeVisible();

      // Paso 7: Entonces el formulario muestra la casilla «Habilitado» sin marcar
      //         (lo que devuelve el servidor al releer el registro: el deshabilitado
      //         del paso 4 quedó persistido). Esta aserción es además la que hace que
      //         el `check()` del paso 8 no pueda ser un no-op silencioso.
      await expect(habilitado).not.toBeChecked();

      // Paso 8: Cuando marca la casilla «Habilitado».
      await habilitado.check();
      await expect(habilitado).toBeChecked();

      // Paso 9: Y pulsa «Guardar».
      await guardarYVolverAlListado(page);
