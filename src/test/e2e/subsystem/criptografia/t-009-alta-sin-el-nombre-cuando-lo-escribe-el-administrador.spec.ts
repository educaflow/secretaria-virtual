...
// origen: ESC-003  |  verifica: V-CertificadoDigital-001
// fuente: .sdd/drafts/2026-09-08_02-53_nombre-apellidos-certificado-digital/test-e2e-desc/t-009-alta-sin-el-nombre-cuando-lo-escribe-el-administrador.desc.md
...
// «Nombre» vacío y editable y el rechazo venga de la validación del nombre
// obligatorio, no del autorrelleno; cambiarlo destruiría lo que el test verifica
...
// Los apellidos que teclea el administrador. El «Nombre» se deja VACÍO: es
...
const CAMPO_EN_ERROR = 'nombre:';
const MENSAJE_ERROR = 'El nombre es obligatorio';
...
// el nombre obligatorio, verificando algo distinto de lo que dice el escenario) y
// como teardown en el `finally` (si una regresión llegara a guardar la fila, el
// siguiente run no se envenena).
...
test.describe('Certificados digitales', () => {
  test('Alta sin el nombre cuando lo escribe el administrador', async ({ page }) => {
    ...
    try {
      ...
      // es de ningún usuario, el autorrelleno no escribe nada y «Nombre» queda
      ...
      // Paso 6: Y deja el campo «Nombre» vacío y escribe en «Apellidos» el valor
      //         «García López». (El nombre accesible real es «Nombre ?» /
      //         «Apellidos ?» por el icono de ayuda; el match por subcadena lo
      //         cubre. Se comprueba que «Nombre» está de verdad vacío antes de
      //         guardar: si el autorrelleno lo hubiera escrito, el alta no
      //         probaría el caso de error que describe el escenario.)
      const nombre = page.getByRole('textbox', { name: 'Nombre' });
      await expect(nombre).toHaveValue('');
      await page.getByRole('textbox', { name: 'Apellidos' }).fill(APELLIDOS);

      // Paso 7: Y pulsa «Guardar».
      await page.getByRole('button', { name: 'Guardar' }).click();

      // Resultado esperado 1: el sistema muestra el error «El nombre es
      // obligatorio». Axelor lo presenta en un diálogo «Validation Error» con un
