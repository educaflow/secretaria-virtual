...
// origen: ESC-020  |  verifica: V-Correo-006
...
test.describe('Administración de correos', () => {
  test('Alta con «para» de formato inválido', async ({ page }) => {
    ...
    const asunto = `Convocatoria de reunión t012-${Date.now()}`;
    ...
    // «CIPFP Mislata», el «para» «direccion-sin-arroba», el asunto «Convocatoria de
    // reunión», el cuerpo «texto» y elige el centro «CIPFP Mislata».
    ...
    await page.getByLabel('Para', { exact: false }).fill('direccion-sin-arroba');
    ...
    // Resultado esperado: el sistema muestra el mensaje "El «para» debe contener
    // direcciones de correo válidas (por ejemplo, usuario@dominio.com)" (diálogo
    // modal de "Validation Error") y no crea el correo.
    await expect(page.getByRole('heading', { name: 'Validation Error' })).toBeVisible();
    await expect(
      page.getByText('El «para» debe contener direcciones de correo válidas (por ejemplo, usuario@dominio.com)')
    ).toBeVisible();
