...
// origen: ESC-025  |  verifica: V-Correo-008
// fuente: .sdd/drafts/2026-06-30_13-56_subsistema-correos/test-e2e-desc/t-017-alta-con-en-copia-oculta-de-formato-invalido.desc.md
test.describe('Administración de correos', () => {
  test('Alta con «en copia oculta» de formato inválido', async ({ page }) => {
    ...
    const asunto = `Convocatoria de reunión t017-${Date.now()}`;
    ...
    // «CIPFP Mislata», el «para» «alumno1@mislata.es», el «en copia oculta»
    // «oculta-sin-arroba», el asunto «Convocatoria de reunión», el cuerpo «texto» y
    // elige el centro «CIPFP Mislata».
    // «En copia oculta» se localiza con exact:true porque el formulario también
    // tiene un campo «En copia» cuyo label es prefijo del anterior (ambigüedad).
    ...
    await page.getByLabel('En copia oculta', { exact: true }).fill('oculta-sin-arroba');
