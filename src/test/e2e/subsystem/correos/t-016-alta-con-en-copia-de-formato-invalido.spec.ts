...
// origen: ESC-024  |  verifica: V-Correo-007
// fuente: .sdd/drafts/2026-06-30_13-56_subsistema-correos/test-e2e-desc/t-016-alta-con-en-copia-de-formato-invalido.desc.md
test.describe('Administración de correos', () => {
  test('Alta con «en copia» de formato inválido', async ({ page }) => {
    ...
    const asunto = `Convocatoria de reunión t016-${Date.now()}`;
    ...
    await page.getByLabel('En copia', { exact: true }).fill('copia-sin-arroba');
    ...
    // Resultado esperado: el sistema muestra el mensaje "El «en copia» debe contener
    // direcciones de correo válidas" (diálogo modal de "Validation Error") y no crea
    // el correo.
    await expect(page.getByRole('heading', { name: 'Validation Error' })).toBeVisible();
    await expect(
      page.getByText('El «en copia» debe contener direcciones de correo válidas')
    ).toBeVisible();
