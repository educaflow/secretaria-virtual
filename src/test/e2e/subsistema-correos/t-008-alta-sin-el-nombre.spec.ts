...
// origen: ESC-014  |  verifica: V-Correo-003
// fuente: .sdd/drafts/2026-06-30_13-56_subsistema-correos/test-e2e-desc/t-008-alta-sin-el-nombre.desc.md
test.describe('Administración de correos', () => {
  test('Alta sin el nombre', async ({ page }) => {
    ...
    const asunto = `Convocatoria de reunión t008-${Date.now()}`;
    ...
    // Paso 2: Cuando rellena el DNI «86862719E», los apellidos «CIPFP Mislata», el
    // «para» «alumno1@mislata.es», el asunto «Convocatoria de reunión», el cuerpo
    // «texto», elige el centro «CIPFP Mislata» y deja vacío el nombre.
    ...
    // El nombre se deja deliberadamente vacío (no se rellena).

    // Paso 3: Y pulsa "Guardar"
    await page.getByRole('button', { name: 'Guardar' }).click();

    // Resultado esperado: el sistema muestra el mensaje "El nombre es obligatorio"
    // y no crea el correo.
