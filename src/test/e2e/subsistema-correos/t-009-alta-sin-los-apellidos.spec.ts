...
// origen: ESC-015  |  verifica: V-Correo-004
// fuente: .sdd/drafts/2026-06-30_13-56_subsistema-correos/test-e2e-desc/t-009-alta-sin-los-apellidos.desc.md
test.describe('Administración de correos', () => {
  test('Alta sin los apellidos', async ({ page }) => {
    ...
    const asunto = `Convocatoria de reunión t009-${Date.now()}`;
    ...
    // Paso 2: Cuando rellena el DNI «86862719E», el nombre «Alumno1», el «para»
    // «alumno1@mislata.es», el asunto «Convocatoria de reunión», el cuerpo
    // «texto», elige el centro «CIPFP Mislata» y deja vacíos los apellidos.
    ...
    // Los apellidos se dejan deliberadamente vacíos (no se rellenan).

    // Paso 3: Y pulsa "Guardar"
    await page.getByRole('button', { name: 'Guardar' }).click();

    // Resultado esperado: el sistema muestra el mensaje "Los apellidos son
    // obligatorios" y no crea el correo.
