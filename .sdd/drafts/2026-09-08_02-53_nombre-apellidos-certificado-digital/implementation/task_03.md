...
Un único componente lógico: el **servicio `CertificadoDigital`** — la interfaz, su implementación y el tipo propio que la nueva acción devuelve. Son tres ficheros fuertemente acoplados (la interfaz declara la acción, el `record` es su tipo de retorno y la `*Impl` la implementa), así que van en la misma tarea.

Regla de negocio compleja referenciada (NO se copia aquí su contenido; **MUST** leerse en su fichero):
...
Las dos filas `Acción: Modificar` (interfaz e implementación) apuntan a ficheros que **ya existen**: antes de tocarlos aplica la **comprobación de conservación** de `implementation.md` §3 — el diseño enumera explícitamente qué se conserva en cada uno (Paso 3 y Paso 4) y **MUST NOT** perderse nada más de lo que declaran las «Eliminaciones declaradas».
...
### Trazabilidad Origen spec → V/R → ubicación (verbatim, las filas de esta tarea)
...
### Eliminaciones declaradas que afectan a esta tarea (verbatim)
