...
- **Centros**: «IES Mislata» (curso académico 2024) e «IES Benicalap» (curso académico 2024). El campo `curso` (curso académico, entero) de cada centro = `2024`.
...
- **Usuarios** (subsistema `common`, con su `CentroUsuario` + `CentroUsuarioTipoUsuario`):
  - Un **Administrador** (tipo ADMINISTRADOR).
  - Un **Supervisor** del centro «IES Mislata» (tipo SUPERVISOR).
...
| supervisor.mislata | educaflow | Supervisor | IES Mislata |
| juan.perez | educaflow | Alumno | IES Mislata |
| ana.lopez | educaflow | Alumno | IES Mislata |
| luis.gil | educaflow | Alumno | IES Mislata |
| marta.ruiz | educaflow | Alumno | IES Mislata |
| pedro.sanz | educaflow | Profesor | IES Mislata |
| sara.vidal | educaflow | Alumno | IES Benicalap |

> Convención adoptada por el diseño (el spec no fija credenciales): logins `nombre.apellido` en minúscula sin acentos; contraseña común `educaflow` salvo el administrador (`admin`/`admin`). Documentado aquí para que `/sdd-test-e2e` pueda iniciar sesión.
...
### Precondiciones
- El usuario `supervisor.mislata` ha iniciado sesión.
...
5. **Y** observa que el campo "Centro" muestra "IES Mislata" en solo lectura y el "Curso académico" muestra "2024" en solo lectura.
6. **Y** pulsa "Guardar".
7. **Y** en el panel "Alumnos" pulsa "Añadir alumno", elige "Juan Pérez" y guarda; repite con "Ana López".

### Resultado esperado
- El grupo "1º DAM A" se crea en estado "Abierto", con centro "IES Mislata" y curso académico "2024".
- El panel "Módulos" muestra "Programación" y "Bases de datos".
- El panel "Alumnos" muestra "Juan Pérez" y "Ana López", cada uno con nota media "Sin nota".
...
### Precondiciones
- El usuario `supervisor.mislata` ha iniciado sesión.
- Existe un grupo "1º DAM A" (curso "1º DAM") en "IES Mislata".
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Juan Pérez".
2. **Y** crea el grupo "1º DAM B" (curso "1º DAM").
3. **Cuando** en el panel "Alumnos" de "1º DAM B" pulsa "Añadir alumno", elige a "Juan Pérez" y guarda.
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Juan Pérez" y "Ana López".
2. **Cuando** en el panel "Alumnos" quita a "Ana López".
3. **Y** guarda el grupo.

### Resultado esperado
- "Ana López" desaparece del grupo junto con sus notas.
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Juan Pérez".
2. **Cuando** entra en el grupo, abre el módulo "Programación", abre la nota de "Juan Pérez", pone el valor "8".
3. **Y** pulsa "Guardar".

### Resultado esperado
- La nota de "Juan Pérez" en "Programación" queda en "8".
- Se registra la "Fecha de calificación".
- En el panel "Alumnos", la nota media de "Juan Pérez" pasa a "8".
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM", módulos "Programación" y "Bases de datos") y le añade a "Juan Pérez".
2. **Cuando** pone a "Juan Pérez" la nota "Matrícula de Honor" en "Programación" y deja "Bases de datos" en "No evaluado".
3. **Entonces** la nota media de "Juan Pérez" es "10".
4. **Y cuando** pone "Bases de datos" en "7".
5. **Entonces** la nota media de "Juan Pérez" pasa a "9".
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Ana López" sin ponerle notas.
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Juan Pérez", "Ana López", "Luis Gil" y "Marta Ruiz".
2. **Y** en el módulo "Programación" pone "Matrícula de Honor" a "Juan Pérez", "Ana López" y "Luis Gil".
3. **Cuando** intenta poner "Matrícula de Honor" a "Marta Ruiz" en "Programación" y guarda.

### Resultado esperado
- El sistema muestra "No se pueden poner más de 3 matrículas de honor en un módulo".
- La nota de "Marta Ruiz" no cambia.
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM"), le añade a "Juan Pérez" y le pone un "8" en "Programación".
2. **Cuando** pulsa "Cerrar grupo".
3. **Y** intenta cambiar la nota de "Juan Pérez" en "Programación".
...
2. **Cuando** pulsa "Nuevo grupo", escribe "1º SMR A", elige el centro "IES Benicalap", el curso académico "2024" y el curso "1º SMR".
3. **Y** pulsa "Guardar".

### Resultado esperado
- El grupo "1º SMR A" se crea en estado "Abierto" en el centro "IES Benicalap" con curso académico "2024".
...
### Precondiciones
- El usuario `supervisor.mislata` y el usuario `admin` figuran en la tabla de acceso.

### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM"), le añade a "Juan Pérez" y lo cierra; después cierra sesión.
...
### Precondiciones
- El supervisor `supervisor.mislata` y el alumno `juan.perez` figuran en la tabla de acceso.

### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM"), le añade a "Juan Pérez" y le pone un "8" en "Programación" y un "6" en "Bases de datos"; después cierra sesión.
2. **Cuando** el alumno "Juan Pérez" inicia sesión y abre "Mis notas".
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM"), le añade a "Juan Pérez" y le pone un "8" en "Programación"; después cierra sesión.
2. **Cuando** el alumno "Juan Pérez" inicia sesión, abre "Mis notas", entra en su grupo y abre la nota de "Programación".
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Juan Pérez".
2. **Cuando** vuelve a pulsar "Añadir alumno", elige otra vez a "Juan Pérez" y guarda.

### Resultado esperado
- "Juan Pérez" sigue apareciendo una sola vez en el grupo (no se añade de nuevo).
...
### Resultado esperado
- El selector ofrece a "Juan Pérez", "Ana López", "Luis Gil" y "Marta Ruiz" (alumnos de "IES Mislata").
...
### Precondiciones
- El usuario `admin` y el usuario `supervisor.mislata` figuran en la tabla de acceso.

### Pasos
1. **Dado** que el administrador crea el grupo "1º SMR A" en el centro "IES Benicalap" (curso académico "2024", curso "1º SMR") desde "Grupos (administración)"; después cierra sesión.
2. **Y** el supervisor de "IES Mislata" inicia sesión y crea el grupo "1º DAM A" (curso "1º DAM").
3. **Cuando** el supervisor abre "Grupos".

### Resultado esperado
- El listado muestra "1º DAM A" (de "IES Mislata").
- El listado NO muestra "1º SMR A" (de "IES Benicalap").
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Juan Pérez".
2. **Cuando** abre la nota de "Juan Pérez" en "Programación", pone "8" y guarda.
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Juan Pérez".
2. **Cuando** abre la nota de "Juan Pérez" en "Programación" en la pantalla y despliega el selector de "Valor".
...
### Pasos
1. **Dado** que el supervisor crea el grupo "1º DAM A" (curso "1º DAM") y le añade a "Juan Pérez".
2. **Y** crea el grupo "1º DAM B" (curso "1º DAM") y le añade a "Ana López" (sin añadir a "Juan Pérez"); después cierra sesión.
3. **Cuando** el alumno "Juan Pérez" inicia sesión y abre "Mis notas".
