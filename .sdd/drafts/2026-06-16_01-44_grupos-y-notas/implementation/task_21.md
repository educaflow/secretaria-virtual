...
para la clase com.educaflow.system.gruposnotas.service.impl.ModuloGrupoServiceImpl.
...
- La descripción es el contrato: implementa EXACTAMENTE los tests que describe (nombre, propósito, mocks,
  ...
  que la descripción no liste ni omitir ninguno. La sección concreta es **"Clase: `com.educaflow.system.gruposnotas.service.impl.ModuloGrupoServiceImpl`"** de `design/test-unit-desc.md`.
...
> Convenciones (de `design/test-unit-desc.md`): JUnit 5 + Mockito (`MockitoExtension`); colaboradores a mockear: `Repository<ModuloGrupo>`; entidades `Grupo`/`Modulo`/`ModuloGrupo` con `new`; aserciones con `org.junit.jupiter.api.Assertions`; nombres `metodo_condicion_resultadoEsperado`.
