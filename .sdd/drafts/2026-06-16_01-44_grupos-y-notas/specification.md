...
- ESC-001 — Crear un grupo con alumnos:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Abre «Grupos», pulsa «Nuevo grupo», escribe el nombre «1º DAM A» y elige el curso «1º DAM».
  3. Guarda el grupo.
  4. El sistema crea el grupo en estado ABIERTO, le fija el centro «IES Mislata» y el curso académico del centro, y rellena automáticamente sus módulos con los del curso «1º DAM» («Programación» y «Bases de datos»).
  5. En el panel de alumnos del grupo, el supervisor añade a los alumnos «Juan Pérez» y «Ana López», eligiéndolos entre los usuarios del centro.
...
- ESC-004 — Un alumno no puede estar en dos grupos del mismo curso académico:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade al alumno «Juan Pérez».
  3. Crea el grupo «1º DAM B» con el curso «1º DAM».
  4. En el panel de alumnos de «1º DAM B» intenta añadir también a «Juan Pérez».
...
- ESC-005 — Quitar un alumno de un grupo abierto:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade a «Juan Pérez» y «Ana López».
  3. En el panel de alumnos quita a «Ana López».
  4. El sistema elimina a «Ana López» del grupo junto con todas sus notas, y «Juan Pérez» permanece.
...
- ESC-019 — No se puede añadir dos veces el mismo alumno al grupo:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade al alumno «Juan Pérez».
  3. En el panel de alumnos vuelve a pulsar «Añadir alumno», elige otra vez a «Juan Pérez» y guarda.
  4. El sistema no lo añade de nuevo: «Juan Pérez» sigue apareciendo una sola vez en el grupo.
- ESC-020 — El selector de alumno solo ofrece alumnos del centro del grupo:
  ...
  4. El sistema ofrece a los alumnos de «IES Mislata» («Juan Pérez», «Ana López», «Luis Gil», «Marta Ruiz») pero no ofrece a «Pedro Sanz» (profesor de «IES Mislata») ni a «Sara Vidal» (alumna de «IES Benicalap»), de modo que ninguno de los dos se puede añadir al grupo.
- ESC-021 — El supervisor solo ve los grupos de su centro:
  1. El administrador inicia sesión, abre «Grupos (administración)», pulsa «Nuevo grupo» y crea el grupo «1º SMR A» con el centro «IES Benicalap», el curso académico «2024/2025» y el curso «1º SMR».
  2. El supervisor del centro «IES Mislata» inicia sesión y crea el grupo «1º DAM A» con el curso «1º DAM».
  3. El supervisor abre «Grupos».
  4. El sistema muestra el grupo «1º DAM A» (de «IES Mislata») pero no muestra el grupo «1º SMR A» (de «IES Benicalap»).
...
- ESC-006 — Poner una nota numérica:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade a «Juan Pérez».
  3. Entra en el grupo, abre el módulo «Programación», abre la fila del alumno «Juan Pérez» y pone la nota 8.
  4. Guarda la nota.
  5. El sistema deja la nota de «Juan Pérez» en «Programación» con valor 8, registra la fecha de calificación y actualiza la nota media de «Juan Pérez» a 8.
- ESC-007 — La nota media excluye los módulos no evaluados y cuenta la matrícula de honor como 10:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» (módulos «Programación» y «Bases de datos») y le añade a «Juan Pérez».
  3. Pone a «Juan Pérez» la nota «Matrícula de Honor» en «Programación» y deja «Bases de datos» en «No evaluado».
  4. El sistema calcula la nota media de «Juan Pérez» como 10 (la matrícula de honor cuenta como 10 y el módulo no evaluado no se cuenta).
...
- ESC-008 — Media «Sin nota» cuando no hay ningún módulo evaluado:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade a «Ana López» sin ponerle ninguna nota.
  3. Consulta el panel de alumnos del grupo.
  4. El sistema muestra a «Ana López» con nota media «Sin nota».
- ESC-009 — Máximo tres matrículas de honor por módulo:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade a cuatro alumnos: «Juan Pérez», «Ana López», «Luis Gil» y «Marta Ruiz».
  3. En el módulo «Programación» pone «Matrícula de Honor» a «Juan Pérez», «Ana López» y «Luis Gil».
  4. Intenta poner «Matrícula de Honor» también a «Marta Ruiz» en «Programación».
  5. El sistema muestra «No se pueden poner más de 3 matrículas de honor en un módulo» y no cambia la nota de «Marta Ruiz».
- ESC-022 — Modificar una nota ya puesta rellena la fecha de última modificación:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade a «Juan Pérez».
  3. Entra en el grupo, abre el módulo «Programación», abre la nota de «Juan Pérez», pone la nota 8 y guarda.
  4. El sistema deja la nota de «Juan Pérez» en «Programación» con valor 8 y la fecha de última modificación vacía.
...
- ESC-023 — Poner un valor de nota inválido:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade a «Juan Pérez».
  3. Entra en el grupo, abre el módulo «Programación», abre la nota de «Juan Pérez», pone el valor 11 y guarda.
...
- ESC-010 — Cerrar un grupo bloquea sus notas y sus alumnos:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM», le añade a «Juan Pérez» y le pone un 8 en «Programación».
  3. Pulsa «Cerrar grupo».
  4. El sistema pasa el grupo a CERRADO y registra la fecha de cierre.
  5. El supervisor intenta cambiar la nota de «Juan Pérez» en «Programación».
...
- ESC-012 — El administrador crea un grupo en otro centro eligiendo centro y curso académico:
  ...
  2. Abre «Grupos (administración)», pulsa «Nuevo grupo», escribe el nombre «1º SMR A», elige el centro «IES Benicalap», el curso académico «2024/2025» y el curso «1º SMR».
  3. Guarda el grupo.
  4. El sistema crea el grupo en estado ABIERTO en el centro «IES Benicalap» con el curso académico «2024/2025» y sus módulos.
- ESC-013 — El administrador reabre un grupo cerrado:
  1. El supervisor del centro «IES Mislata» inicia sesión, crea el grupo «1º DAM A» con el curso «1º DAM», le añade a «Juan Pérez» y lo cierra.
  2. El administrador inicia sesión, abre «Grupos (administración)» y entra en el grupo «1º DAM A» (que está CERRADO).
...
- ESC-014 — El alumno consulta sus notas y su nota media:
  1. El supervisor del centro «IES Mislata» inicia sesión, crea el grupo «1º DAM A» con el curso «1º DAM», le añade al alumno «Juan Pérez» y le pone un 8 en «Programación» y un 6 en «Bases de datos».
  2. El alumno «Juan Pérez» inicia sesión y abre «Mis notas».
...
- ESC-015 — El alumno no puede modificar sus notas:
  1. El supervisor del centro «IES Mislata» inicia sesión, crea el grupo «1º DAM A» con el curso «1º DAM», le añade a «Juan Pérez» y le pone un 8 en «Programación».
  2. El alumno «Juan Pérez» inicia sesión, abre «Mis notas», entra en su grupo y abre la nota de «Programación».
...
- ESC-026 — El alumno no ve los grupos a los que no pertenece:
  1. El supervisor del centro «IES Mislata» inicia sesión.
  2. Crea el grupo «1º DAM A» con el curso «1º DAM» y le añade al alumno «Juan Pérez».
  3. Crea un segundo grupo «1º DAM B» con el curso «1º DAM» y le añade a la alumna «Ana López» (sin añadir a «Juan Pérez»).
  4. El alumno «Juan Pérez» inicia sesión y abre «Mis notas».
...
- Al menos dos centros de prueba: «IES Mislata» y «IES Benicalap», cada uno con su curso académico.
- En el catálogo educativo, los cursos «1º DAM» (con los módulos «Programación» y «Bases de datos») y «1º SMR».
- Usuarios de prueba con acceso: un administrador, un supervisor del centro «IES Mislata», varios usuarios de tipo Alumno del centro «IES Mislata» («Juan Pérez», «Ana López», «Luis Gil», «Marta Ruiz»), un usuario de tipo Profesor del centro «IES Mislata» («Pedro Sanz») y un usuario de tipo Alumno del centro «IES Benicalap» («Sara Vidal»).
