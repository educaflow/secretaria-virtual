...
| R-004* | El sistema resuelve el `TipoUsuario` para los usuarios autorizados que cree usando un mapeo explícito desde el valor del enum del tipo de fichero al código del `TipoUsuario` (en esta iniciativa: `PROFESOR_EXTERNO` (enum) → código `PROFESOR_EXTERNO` (TipoUsuario)). Si el código no existe en BD, aborta el procesamiento, marca la tarea con `estado=false` y registra en el log "Importación abortada: error de configuración, no existe el tipo de usuario con código 'PROFESOR_EXTERNO'." | TareaImportacion | Procesamiento del CSV | Antes | Constante técnica de esta iniciativa; el mapeo se ampliará explícitamente cuando se admitan más tipos importables por CSV. |
...
- **A1*** (R-004): el mapeo entre los valores del enum `TipoFicheroImportacion` y los códigos de `TipoUsuario` se documenta como tabla explícita en el subsistema (`PROFESOR_EXTERNO (enum) → "PROFESOR_EXTERNO" (código)`). No se obtiene por convención automática de nombre; se ampliará explícitamente cuando se admitan más tipos importables por CSV.
...
- **A8*** (validez de DNI como invariante de modelo): la validación de formato del DNI **no** se añade como invariante de modelo de `UsuarioAutorizado` en esta iniciativa. El proceso de importación garantiza que los DNIs creados son válidos, pero la entidad en sí no impone esa validación a otras vías de creación. Confirmar si se desea ampliar el alcance.
