...
| `subsystem/common/db/repo/TipoUsuarioRepository.java` | Crear | k-sistemas (modelos.md) | Repositorio concreto necesario por `repository="abstract"` en el XML |
| `subsystem/common/service/TipoUsuarioService.java` | Crear | k-sistemas (servicios.md) | Interfaz del servicio — extiende `ModelService<TipoUsuario>`; expone `findByCodigo` |
| `subsystem/common/service/impl/TipoUsuarioServiceImpl.java` | Crear | k-sistemas (servicios.md) | Implementación con `findByCodigo` delegado al repositorio |
...
| `subsystem/importacion/service/impl/TareaImportacionServiceImpl.java` | Modificar | k-sistemas | Solo eliminar el prefijo del log en `fireActionRule_ejecutarImportacion` |
...
### Paso 3 — Crear `TipoUsuarioRepository`, `TipoUsuarioService` y `TipoUsuarioServiceImpl`

#### 3.1 `TipoUsuarioRepository.java`
...
/**
 * Repositorio concreto de TipoUsuario. Necesario porque la entidad declara
 * repository="abstract" en el XML. No añade métodos adicionales:
 * findByCodigo está generado en AbstractTipoUsuarioRepository.
 */
public class TipoUsuarioRepository extends AbstractTipoUsuarioRepository {
}
```

#### 3.2 `TipoUsuarioService.java`

Fichero: `src/main/java/com/educaflow/subsystem/common/service/TipoUsuarioService.java`

FQN: `com.educaflow.subsystem.common.service.TipoUsuarioService`
...
public interface TipoUsuarioService extends ModelService<TipoUsuario> {
    ...
    /**
     * Busca un TipoUsuario por su código identificador.
     * Usado en R-004 por ImportadorUsuarioCSV para resolver el TipoUsuario
     * cuyo código es "PROFESOR_EXTERNO".
     */
    Optional<TipoUsuario> findByCodigo(String codigo);
}
```

#### 3.3 `TipoUsuarioServiceImpl.java`

Fichero: `src/main/java/com/educaflow/subsystem/common/service/impl/TipoUsuarioServiceImpl.java`

FQN: `com.educaflow.subsystem.common.service.impl.TipoUsuarioServiceImpl`
...
public class TipoUsuarioServiceImpl
    ...
    /**
     * Delega en el finder findByCodigo generado en AbstractTipoUsuarioRepository.
...
**Verificación:** `./gradlew clean build --info`; `ModelServiceFactory` descubre `TipoUsuarioServiceImpl` por convención de paquetes.

---

### Paso 5 — Crear `UsuarioAutorizadoService` e `UsuarioAutorizadoServiceImpl`
...
public class UsuarioAutorizadoServiceImpl
     ...
     * V-002..V-005 (nulidad de centro, dni, tipoUsuario y curso) están cubiertas por
...
### Paso 6 — Crear `UsuarioAutorizadoRepository`
...
### Paso 7 — Modificar `ResultadoImportacion`
...
### Paso 8 — Implementar `ImportadorUsuarioCSV`
...
/**
 * R-010 — Llama a usuarioAutorizadoService.findByCentroDniTipoUsuarioCurso(
 *          ctx.centro(), dniNormalizado, ctx.tipoUsuario(), ctx.curso()).
 *          Si devuelve Optional vacío: crea new UsuarioAutorizado() con los cinco campos
 *          (centro, dniNormalizado, tipoUsuario, curso, fechaExportacion=LocalDateTime.now())
 *          y llama a usuarioAutorizadoService.insert(ua). El importador fija fechaExportacion
 *          porque en el caso XML proviene del propio fichero; el servicio no la sobreescribe.
 *          Incrementa contador.creados. Las creaciones no se añaden al log individualmente.
...
### Paso 9 — Modificar `TareaImportacionServiceImpl`
...
No se añade ningún campo nuevo. El único cambio es en `fireActionRule_ejecutarImportacion`:

```java
/**
 * R-014 / R-015: único cambio respecto al estado anterior:
...
| V-005 | `UsuarioAutorizado.curso` obligatorio | Dominio XML | `UsuarioAutorizado.xml` → `<integer name="curso" required="true"/>` — **nuevo en esta iniciativa** |
...
| R-010 | DNI válido + no existe → crear UsuarioAutorizado con fechaExportacion=now() | Importador | `ImportadorUsuarioCSV.procesarDni()` → construye `UsuarioAutorizado` con los cinco campos (centro, dniNormalizado, tipoUsuario, curso, `fechaExportacion=LocalDateTime.now()`) y llama a `svc.insert(ua)`; el servicio no sobreescribe `fechaExportacion` |
...
1. **fechaExportacion de UsuarioAutorizado**: la asigna el **importador** antes de llamar a `svc.insert()`, no el servicio. El patrón es consistente con `ImportadorUsuarioXML`, donde `fechaExportacion` proviene del dato del propio fichero XML, no de un timestamp del sistema. Para el CSV se usa `LocalDateTime.now()` como valor, pero la responsabilidad de asignarlo sigue siendo del importador. El servicio no sobreescribe ni calcula este campo.
