...
Si este skill se ejecuta **dentro de un subagente** (lo invocan `sdd-implementer` o `sdd-test-e2e` vía `Agent`), no hay usuario al que preguntar: ante `NEEDS_CONTEXT` o `BLOCKED`, **devuelve el estado de bloqueo con su descripción detallada como resultado final** en vez de `AskUserQuestion`/esperar — el orquestador padre decide.
