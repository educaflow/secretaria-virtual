...
`/sdd-debug-with-test-e2e-desc` lee este fichero una vez implementado el código: trocea los tests en una carpeta `test-e2e-desc/`, traduce cada escenario a comandos `playwright-cli` y ejecuta un **bucle de auto-corrección**: si un test falla, corrige el código y reintenta.
...
- **Lenguaje**: usar `Dado` / `Cuando` / `Y` / `Entonces` (o `Given` / `When` / `And` / `Then`). No usar selectores CSS, refs `eN`, ni comandos `playwright-cli`. La traducción a comandos la hace `/sdd-debug-with-test-e2e-desc`.
