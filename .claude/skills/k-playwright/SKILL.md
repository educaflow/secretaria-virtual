...
| `conventions.md` | Convenciones del proyecto: estructura `src/test/e2e/<capa>/<sistema>/` espejo del código (pares descripción/test emparejados por nombre base, `AAAA.desc.md` ↔ `AAAA.spec.ts`; `.plan.md` deprecado), helper `_support/auth.ts`, `seed.spec.ts`, baseURL, login común. |
