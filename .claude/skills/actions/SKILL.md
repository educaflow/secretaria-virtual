...
```
 ...
 - Para indicar el formularioa a mostrar, indicar el atributo `<view type="form" name="Nombre del grid"/>` 
 - En caso de que sea un grid que llama a un form, es **obligatorio** que vaya antes del grid que el formulario
 ...
 - Añadir `<view-param name="forceEdit" value="true"/>` para que para permitir que el formulario se abra en modo edición, para ello usar en el grid `canEditOnClick="true"` para que al hacer click en la fila se abra el formulario directamente en edición
...
```
 ...
 - En este caso el método Java entrá como argumentos `ActionRequest actionRequest, ActionResponse actionResponse`
```java
   @CallMethod
...
```
 ...
 - En este otro ejemplo, el método Java `enviarCorreo` recibe como argumento el id del registro actual. Para ello se usa la sintaxis `method="enviarCorreo(id)"` y el método Java entrará como argumento `Long id`:
```java   @CallMethod
...
<action-attrs name="subsysSistemaEducativo.LeyEducativa@Main-set-orderDate-readOnly-confirmed-action">
  <attribute for="orderDate" name="readonly" expr="confirmed" />
</action-attrs>
```

- El atributo `for` indica el campo al que se le van a cambiar los atributos.
...
```xml
<action-record name="subsysSistemaEducativo.Ciclo.Curso-onNew-action" model="com.educaflow.subsystem.sistemaeducativo.db.Curso">
...
### `<action-group>` — secuencia de validaciones local + remota

```xml
<action-group name="subsysSistemaEducativo.LeyEducativa@Main-validateSave-action">
...
```
 ...
 - Simplmente se listan las acciones a ejecutar en orden. En este caso, primero se ejecuta la acción de validación local (`subsysSistemaEducativo.LeyEducativa@Main-Local-validateSave-action`) y si pasa sin errores, se ejecuta la acción de validación remota (`subsysSistemaEducativo.LeyEducativa@Main-Remote-validateSave-action`).
...
### `<action-condition>`
Validaciones a nivel de campo concreto, que muestran el mensaje de error justo debajo del campo. Por ejemplo, para validar que el campo `motivoRechazo` no esté vacío:
...
```
 ...
 - El atributo `if` indica la expresión que se va a evaluar para validar el campo. Si la expresión es `false`, se muestra el mensaje de error.
 - El atributo `error` indica el mensaje de error que se va a mostrar debajo del campo si la validación falla.
 - Si no se indica el atributo `if` se comprueba que no esté vacio.
...
```xml
<action-script
        name="subsysVentas.Factura@Pendiente-Remote-guardarFactura-action"
...
insisto en que no es recom


## Convenciones de nombres para las acciones

```
{Prefijo}.{Entidad}@{Vista}-[Local-|Remote-]{operación}-action
```
...
| `Local-`      | Prefijo para validaciones client-side (`action-validate`, `action-condition`) | `Local-validateSave`                         |
| `Remote-`     | Prefijo para llamadas al servidor (`action-method` o `action-script`)        | `Remote-validateSave`                        |
| `{operación}` | Descripción en camelCase de lo que hace                                       | `validateSave`, `marcarComoFirmada`          |
...
- **`action-group`** — orquestador público, sin prefijo `Local`/`Remote`:
  `subsysSistemaEducativo.LeyEducativa@Main-validateSave-action`
...
- **`action-record`** — describe campo y valor con `set-{campo}-{valor}`:
  `subsysFirma.TareaFirma@Pendiente-set-pasoActual-paso1Inicio-action`
...
Las acciones se colocan tras el formulario que las usa. Y se sigue este orden estricto para facilitar la lectura y el mantenimiento:

 - Acciones de las tareas principales: `<action-group>`
 - Acciones de Validaciones en local: `<action-condition>` y `<action-validate>`
...
Antes de ello se pondrán exactamente estos comentarios (incluyendo exactamente tantos * como se indica) y que sirven para marcar claramente cada sección de acciones:
