...
## Tipos de menuitems y su ubicación

- **Menuitems raíz** (sin `action`): en el fichero `src/main/java/com/educaflow/secretariavirtual/menus/menus.xml`
- **Menuitems hoja** (con `action`): en el fichero `src/main/java/com/educaflow/secretariavirtual/menus/menus.xml`
...
- Para cada menuitem hoja: añadirlo justo antes de la `<action-view>` correspondiente en el fichero de vistas.
...
- Decidir el nombre del menuitem raíz siguiendo la convención de nomenclatura: `{Prefijo}[-menuitem | .{Entidad}@{Vista}-menuitem | -{concepto}-menuitem]`
...
- [ ] Revisar que los menuitems están en `src/main/java/com/educaflow/secretariavirtual/menus/menus.xml` 
