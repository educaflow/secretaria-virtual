...
- **MUST NOT** escribirse a mano la `<permission name="<Entidad>.all">` del tipo de expediente, ni en `auth-expedientes.xml` ni en ningún otro `auth-*.xml`: la genera el build en el `auth-<Code>.xml` del data-init del tipo, ya enganchada a los grupos `admins` y `users`. Escribirla a mano la duplica. Se genera con `create/read/write/remove` **sin `condition`**, que es un agujero conocido documentado en `CLAUDE.md`; **MUST NOT** taparse añadiéndole aquí una `condition` suelta.
