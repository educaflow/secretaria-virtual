...
- **Sí va en el diseño:**
  ...
  - Para cada regla `V-XXX` y regla de negocio: en qué capa va (cliente/servidor/dominio), en qué método o acción concreta, y un comentario de su lógica. **No el código.**
  - Trazabilidad `V-XXX → ubicación` (clase + método o fichero XML + nombre de acción).
...
**Regla de cobertura total (obligatoria):** **TODAS** las validaciones (`V-XXX`) y reglas de negocio del análisis deben quedar **ubicadas** en el diseño: cada regla debe tener una entrada en la matriz de trazabilidad apuntando a un método o acción concreta del diseño, con un comentario que describa su lógica. Si alguna `V-XXX` o regla de negocio no tiene ubicación asignada, el diseño está incompleto y no se puede guardar.
...
- **Tarea 2 del subagente — Detallar contenido del diseño y trazabilidad**:
  ...
  - Construir la matriz `V-XXX → ubicación` (clase.método o fichero XML + nombre de acción) que demuestre que **toda regla `V-XXX` y regla de negocio del análisis está ubicada** en al menos una clase/método o acción del diseño, con su capa indicada (cliente/servidor/dominio).
...
**Reglas específicas para el paso de Servicios (validaciones y reglas de negocio):**

El paso de servicios debe listar las firmas de `validateInsert` y `validateUpdate` (y de los `fireActionRule_*` y métodos de negocio) **con un comentario en el cuerpo** que describa, para cada regla del análisis ubicada en ese método:

1. **Identificador de la regla** (`V-XXX` o nombre de la regla de negocio del análisis).
2. **Lógica resumida** (qué se comprueba o qué se hace).
3. **Contenido del mensaje de error** descrito por lo que debe transmitir (debe incluir el valor recibido y los valores válidos cuando sea posible) — **no el literal del mensaje**.
4. **Si la información de los valores válidos viene de BD**, indicarlo en el comentario (qué fuente — catálogo, repositorio, etc.).
...
// Método:
public BusinessMessages validateInsert(Bar entidad) throws BusinessException;
//   Aplica:
...
- **Cobertura total obligatoria:** **TODAS** las validaciones (`V-XXX`) y reglas de negocio del análisis deben tener una **ubicación asignada** en el diseño (clase + método o fichero XML + acción) con un **comentario descriptivo** de su lógica. Ninguna regla puede quedar sin ubicación. El diseño debe poder leerse de forma que para cada `V-XXX` se vea explícitamente dónde irá implementada y qué tiene que hacer.
...
- **Mapeo de validaciones a la capa correcta:** validaciones de campo individual y entre campos del mismo registro → cliente (`action-validate`/`action-condition` en vistas, declaradas por nombre en el diseño); integridad entre registros, ciclo de vida y reglas de negocio → servidor (`validateInsert`/`validateUpdate`/`fireActionRule_*` en servicios, declaradas por firma + comentario). Cada regla `V-XXX` del análisis debe estar ubicada en al menos una de estas dos capas.
...
- **Trazabilidad obligatoria:** la matriz `V-XXX → ubicación` no es opcional. Cada fila debe apuntar a la clase + método o al fichero XML + nombre de acción que implementará la regla. Si la matriz tiene una fila sin ubicación, el diseño no está terminado.
...
- [ ] ¿Cada método en el paso de servicios tiene un comentario que indica qué reglas `V-XXX` o reglas de negocio aplica, qué lógica ejecuta y qué mensajes de error produce (descritos por su contenido, no su literal)?
- [ ] ¿Cada acción de vista declarada tiene un comentario de su propósito y los campos/condiciones que intervienen?
- [ ] ¿Las validaciones del análisis funcional están mapeadas a la capa correcta? Validaciones de campo individual y entre campos del mismo registro → cliente; integridad entre registros, ciclo de vida y reglas de negocio → servidor.
...
- [ ] **¿TODAS las reglas de negocio del análisis (operaciones, transiciones de estado, campos calculados, efectos secundarios como envío de correos, generación de PDF, registro de entrada/salida…) tienen un método o acción ubicada en el diseño con un comentario que describe qué hace?** Cada regla de negocio debe tener su firma de método o su acción declarada, no sólo descrita en prosa fuera de los pasos.
- [ ] **¿La matriz `V-XXX → ubicación` final del diseño tiene una entrada por cada regla del análisis y cada entrada apunta a una clase + método o fichero XML + nombre de acción?** Si la matriz tiene huecos o entradas sin ubicación, el diseño está incompleto.
...
- [ ] **¿Las validaciones del análisis funcional están mapeadas a la capa correcta?** Validaciones de campo individual y entre campos del mismo registro → cliente; integridad entre registros, ciclo de vida y reglas de negocio → servidor. Ver `k-validaciones` y `k-sistemas/validaciones.md`.
