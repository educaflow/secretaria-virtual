...
- **descomponedor** — **lee `test-e2e-desc.md` y escribe la carpeta `test-e2e/`** (un fichero autocontenido por test + índice) (§2.1).
...
| `decomposition.md` | **Cómo descomponer `test-e2e-desc.md`** en un fichero por test: qué cabecera común copiar en cada uno para que sea autocontenido, las plantillas exactas de `test-e2e-desc_NN.md` y del índice `tests-e2e-desc.md` (con checkbox por test), la numeración y el checklist. | El **descomponedor** (§2.1). |
...
El skill `sdd-test-e2e` lanza estos tres roles. Todos reciben este `README.md`; cada uno recibe además **su** entrada propia (la ruta de su fichero de test, o el `test-e2e-desc.md` en el caso del descomponedor) y **lee un subconjunto distinto de los ficheros de esta carpeta**.
...
|---|---|---|---|---|
| **descomponedor** (§2.1) | **Lee `test-e2e-desc.md` y escribe los ficheros de test** | la ruta del `test-e2e-desc.md` | `decomposition.md` | `{iniciativa}/test-e2e/` con el índice y un fichero por test |
| **ejecutor** (§2.2) | **Pilota un test** en el navegador | la ruta de **un** `test-e2e-desc_NN.md` | `execution.md` | `SUCCESS {id}` o `FAIL {id}` + `=== FALLO ===` (no toca código) |
...
**Tarea:** leer el `test-e2e-desc.md` íntegro y **escribir la carpeta `{iniciativa}/test-e2e/`**: un fichero `test-e2e-desc_NN.md` **autocontenido** por test (cabecera común + bloque del test) y el índice `tests-e2e-desc.md` con un checkbox sin marcar por test.
...
**Tarea:** dado **un** `test-e2e-desc_NN.md`, **ejecutarlo contra la app real** (`http://localhost:8080`, ya levantada por el motor) y reportar `SUCCESS`/`FAIL`.
...
- **Entrada propia:** la ruta de **su** `test-e2e-desc_NN.md` (autocontenido: trae estado inicial + credenciales + el test).
...
- **Entrada propia:** la ruta del `test-e2e-desc_NN.md`, el bloque `=== FALLO ===` del ejecutor y el extracto del log de la app.
...
- **MUST NOT** tocar el contrato: ni `test-e2e-desc.md`, ni los ficheros de `test-e2e/`, ni el XML/contrato del diseño. Si la corrección lo exigiera → `BLOQUEADO`.
...
### 3.2 Salida — `test-e2e/` y el árbol del proyecto
...
- Lánzalo con `Bash`, `run_in_background: true` y `dangerouslyDisableSandbox: true` (`run.sh` escribe en `~/.gradle`, fuera del sandbox), redirigiendo el log al fichero del motor:
  ```bash
  exec ./run.sh > .sdd/drafts/{iniciativa}/test-e2e/app.log 2>&1
  ```
