...
Lo lee el **ejecutor** (§2.2 del `README.md`). Tarea: ejecutar **un** `test-e2e-desc_NN.md` contra la aplicación real y devolver `SUCCESS`/`FAIL`. **NO** modificar código.
...
2. Lee tu `test-e2e-desc_NN.md` (autocontenido): trae el `## Estado inicial de la base de datos` (datos maestros + **tabla de credenciales de login**) y el bloque del test (`Precondiciones`/`Pasos`/`Resultado esperado`).
...
**MUST NOT** modificar `test-e2e-desc_NN.md` ni ningún fichero del proyecto. Solo lees y pilotas.
