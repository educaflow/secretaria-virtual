...
**CRITICAL — antes de nada, mira si la vista está en un paquete exento.** `agent_docs/view-rules.md` deja **fuera del sujeto de todas sus reglas** los paquetes `gestioncentro`, `expedientes`, `tiposexpedientes` y `tramites` (framework propio de expediente/tramitación), y el soporte de los tests lo implementa (`PAQUETES_EXENTOS` en `src/test/java/com/educaflow/views/support/ViewFiles.java`). Eso parte la revisión en dos:
