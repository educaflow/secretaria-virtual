...
1. **Decidir el tipo de skill** (§3) — knowledge (`k-*`) o action (`sdd-*`/`developer-*`/`skill-*`/imperativo).
...
Encapsulan **conocimiento de dominio reutilizable**: convenciones, patrones, vocabulario, decisiones arquitectónicas. Otros skills (sobre todo los `sdd-*`, `developer-*` y `skill-*`) los cargan como referencia.
...
### 3.2 Action skills (`sdd-*`, `developer-*`, `skill-*`, imperativos)
...
- `developer-*` → producen o revisan código real en el árbol del proyecto.
- `skill-*` → **meta**: ayudan a otros skills de forma **abstracta**, sin conocimiento del dominio ni del problema. Dos usos: motores de proceso reutilizables que otros skills invocan con su propio contrato (`skill-orquestador-reviewer`), y skills que evalúan o mejoran otros skills (`skill-eval`, `skill-reviewer`). El conocimiento de cómo se escribe un skill vive en este propio `k-skill`.

**Ejemplos**: `sdd-specification`, `sdd-designer`, `sdd-implementer`, `sdd-close`, `developer-code-implementer`, `developer-code-reviewer`, `skill-orquestador-reviewer`, `skill-eval`, `skill-reviewer`.
...
- Tipo knowledge (`k-*`): documenta convenciones y patrones. Tipo action (`sdd-*`, `developer-*`, `skill-*`): ejecuta proceso con fases numeradas.
