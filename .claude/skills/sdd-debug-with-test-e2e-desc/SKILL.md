...
3. **Fase 2 — Descomponer**: lanzar el subagente **descomponedor**, que escribe `test-e2e-desc/` (índice + un fichero por test); cargar el progreso del índice y descartar los tests ya `[x]` (§7).
...
- En `.sdd/drafts/{iniciativa}/test-e2e-desc/`: el índice `tests-e2e-desc.md` (un checkbox por test, marcado `[x]` al pasar) y un `t-NNN-<slug>.desc.md` autocontenido por test. **Su estructura interna la define la plantilla**, no este skill.
...
- **Contrato de tokens**: literal exacto — `ESCRITO: test-e2e-desc/`, `SUCCESS`/`FAIL`, `CORREGIDO`/`BLOQUEADO`/`DESIGN-ERROR`. Los subagentes **MUST NOT** pegar contenido que ya está en disco.
