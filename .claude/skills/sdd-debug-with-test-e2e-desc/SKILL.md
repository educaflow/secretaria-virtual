...
description: Herramienta de testing E2E del pipeline SDD. Dado el `test-e2e-desc.md` de una iniciativa (la descripción de tests Given/When/Then que produce `/sdd-designer` a partir de los escenarios `ESC-NNN`, propagada por `/sdd-implementer` a `implementation/`), descompone esos tests en una carpeta `test-e2e-desc/` (un índice `tests-e2e-desc.md` con un checkbox por test + un `t-NNN-<slug>.desc.md` autocontenido por test) y luego, test a test, los ejecuta contra la aplicación real. El skill es un MOTOR genérico y agnóstico al artefacto: aporta solo el flujo (localizar la iniciativa, cargar el contrato, descomponer, y por cada test el bucle ejecutar→corregir→reejecutar) y delega TODO lo específico (qué es un test, cómo se descompone, cómo se ejecuta contra la app, cómo se corrige el código) en el `README.md` de la carpeta de plantillas activa, que los subagentes leen como contrato — el skill trae una carpeta `template-<nombre>/` por tipo de artefacto (no conoce sus nombres) y usa la que declare el frontmatter `template:` del `design/design.md` de la iniciativa (heredado de la spec; configurable con `--template-dir`). Lanza tres roles: un descomponedor que escribe `test-e2e-desc/`; un ejecutor por test que lo pilota en el navegador y devuelve SUCCESS/FAIL; y, ante un FAIL, un corrector que analiza el problema, carga los skills necesarios y arregla el código de la aplicación en Java/Kotlin (bucle ejecutar↔corregir, **LIMIT** 10 por test). Marca cada test como `[x]` en el índice al pasar (progreso reanudable). La salida son las correcciones de código en `src/main/...` y la carpeta `test-e2e-desc/` con el estado de cada test.
handoffs:
  - label: Cerrar la iniciativa
    agent: sdd-close
    prompt: Cerrar la iniciativa en .sdd/drafts/{carpeta-iniciativa}/ tras pasar los tests E2E — archivar el draft verbatim en .sdd/archive/.
...
2. **Fase 1 — Cargar** el contrato (`<plantilla-activa>/README.md`) y resolver las rutas de entrada (§5).
...
Un único `test-e2e-desc.md` que vive **siempre en la subcarpeta `implementation/`** de la iniciativa (`/sdd-designer` lo produce en `design/` y `/sdd-implementer` lo propaga a `implementation/`; este skill lee **el de `implementation/`**): una **cabecera común** (los datos previos y las credenciales de login que comparten todos los tests) seguida de un bloque `## T-NNN — <nombre>` por test. El skill **no asume su estructura interna** —qué secciones forman esa cabecera y cómo se escribe cada bloque lo declara `<plantilla-activa>/README.md`—: la conoce el descomponedor leyendo el contrato.
...
- En el **árbol del proyecto** (`src/main/java/com/educaflow/...`): las correcciones de código (Java/Kotlin) necesarias para que los tests pasen.
...
**MUST NOT** modificar `test-e2e-desc.md` ni ningún artefacto de `design/` o `implementation/` para que un test pase. Este skill solo lee la descripción de tests, escribe `test-e2e-desc/` y corrige **código de la aplicación** (Java/Kotlin).
...
src/main/java/com/educaflow/...  ← código de la aplicación (Java/Kotlin) que se corrige
```
...
1. `--template-dir=<ruta>` explícito → esa carpeta (válvula de testing). **ERROR** si apunta a otra carpeta `template-*/` de este skill **distinta** de la que declara la iniciativa (STOP condition).
2. Sin flag → lee la clave `template:` del frontmatter del **`design/design.md`** de la iniciativa (el diseño la hereda de la spec) y usa `template-<valor>/`; si no la trae, léela del `specification.md`; si tampoco → pregúntala con `AskUserQuestion` (una opción por carpeta `template-*/` del skill). Si `template-<valor>/` no existe en este skill → **ERROR** indicando las disponibles o que se pase `--template-dir=`.
...
**REQUIRED — el README es leído por los tres roles** (descomponedor, ejecutor, corrector). Cualquier `README.md` de plantilla (cualquier `template-<nombre>/` del skill o una externa apuntada con `--template-dir=`) **MUST** estar redactado para esos tres roles: debe delimitar, por rol, qué tarea hace y qué ficheros de la plantilla le aplican.
...
### 2.3 No tocar el contrato — solo código de la aplicación

**MUST NOT** modificar `test-e2e-desc.md` ni los ficheros de `test-e2e-desc/` para que un test pase (eso es trampa). **MUST NOT** editar XML de dominios/vistas materializados ni cambiar firmas declaradas por el diseño: si el fallo exige eso, es un **error de diseño** — el corrector devuelve `DESIGN-ERROR`, el motor escribe `test-e2e-desc/error_design.log` y **detiene el skill** sin preguntar (§9.3); la corrección vuelve a `/sdd-designer`. (Si lo que falta es un recurso del **entorno** ajeno al diseño, el corrector devuelve `BLOQUEADO` y el motor **STOP** y pregunta al usuario.) Este skill corrige **código de la aplicación** (Java/Kotlin) según lo que declare el contrato de corrección de la plantilla activa.
...
Cada `t-NNN-<slug>.desc.md` que escribe el descomponedor **MUST** incluir la cabecera común que defina el contrato (los datos previos + las credenciales de login) **además** del bloque del test, porque el ejecutor recibe **solo ese fichero** y necesita las credenciales y el estado previo para pilotar la app sin depender de ningún otro fichero. El contrato (`README.md`) define el formato exacto.
...
1. **REQUIRED — lee con `Read` la guía `<plantilla-activa>/README.md`** (resuelta según §2.1: el frontmatter `template:` del `design/design.md` de la iniciativa, o `--template-dir`): confirma que existe (si no → **ERROR**, STOP condition), entiende a alto nivel qué pide a cada rol, y **lee su sección «Gestión de la app»** (los comandos que el motor ejecutará en la Fase 3/4 — §2.2). El resto del contrato lo leen los subagentes de disco.
...
2. **Lanza el subagente corrector** (`Agent`, `subagent_type: claude`, `run_in_background: false`). Su tarea: analizar la causa, **decidir qué skills necesita y cargarlos**, y corregir el código según el contrato (delegando en `developer-code-implementer` si el contrato lo indica). **MUST NOT** tocar el contrato (§2.3).

   **Prompt del subagente corrector**:

   > Eres un experto arquitecto en Java/Kotlin y el framework Axelor que tiene que **corregir el código para que un test E2E pase**.
   >
   > - **Reglas para la corrección**: lee `{ruta de <plantilla-activa>/README.md}` y los ficheros que referencie —en particular el contrato de **corrección** (cómo localizar la causa, cómo decidir y cargar los skills de dominio necesarios, cómo delegar el código en `developer-code-implementer`, y qué **MUST NOT** tocarse)—. Síguelo.
   ...
   > - **OBLIGATORIO**: analiza qué skills de dominio necesitas para el arreglo **según la sección de skills del contrato de corrección** (es él, y no este prompt, quien declara cuáles aplican a este artefacto y cuáles **MUST NOT** cargarse), **cárgalos con la herramienta `Skill` antes de corregir** y, si el contrato lo indica, delega el código en `developer-code-implementer`.
   ...
   >   - `BLOQUEADO: {T-NNN} — {motivo}` — falta un recurso del entorno ajeno al diseño; no se puede arreglar solo con código de la aplicación.
   >   - `DESIGN-ERROR: {T-NNN} — {motivo detallado}` — el test no se puede hacer pasar sin tocar el diseño (§9.3). Da el **máximo detalle**: qué fichero del diseño, qué es inconsistente o falta y por qué no se puede resolver escribiendo código de la aplicación.
...
Un **error de diseño** es un fallo de test cuyo origen está en el propio diseño (`design/`, los XML/firmas materializados o el propio `test-e2e-desc.md`) y que **NO se puede resolver corrigiendo código de la aplicación**: hay que volver a `/sdd-designer`. Ejemplos: el test espera un método/firma que el diseño no declara, una vista/dominio materializado es inconsistente con lo que el test exige, o el `test-e2e-desc.md` describe un comportamiento que contradice el diseño.
...
## Por qué no se puede resolver con código de la aplicación
{por qué es un fallo del diseño y no del código: el diseño/contrato es fijo y corregir el código no hace pasar el test}
...
- **CRITICAL — agnosticismo**: este SKILL es un **motor de flujo**; **no sabe qué es un test, cómo se descompone, cómo se ejecuta ni cómo se corrige el código**. Todo lo específico lo define `<plantilla-activa>/README.md` (configurable con `--template-dir`), que **leen los subagentes**. Único contrato fijo: entrada `test-e2e-desc.md`, salida en `test-e2e-desc/` y en el árbol del proyecto. Única parte específica que ejecuta el motor: la **gestión de la app** (§2.2, comandos en el README).
...
- **Descomponer** (§7): **un** descomponedor escribe `test-e2e-desc/` (un fichero **autocontenido** por test + índice con checkbox); responde `ESCRITO: test-e2e-desc/` + bloque `=== TESTS ===`. Cada `t-NNN-<slug>.desc.md` incluye la cabecera común (datos previos + credenciales) para que el ejecutor no dependa de otros ficheros (§2.5).
