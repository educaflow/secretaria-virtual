...
- **Siempre** `k-tipo-expediente` — **el skill central**: la carpeta de versión, el `TipoExpedienteInstance.xml` con sus fases y la máquina de estados, y **todos sus ficheros** (modelo, `PhaseEventManager` e `InitialEventManager`, validator, vistas preprocesadas, `documentospdf/` y recetas); su `SKILL.md` los indexa.
