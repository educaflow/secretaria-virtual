...
- **a) Estructura.** (`decisiones.md` y los `log_*.txt` los declara el motor, `sdd-designer` §2.5: no son fallos de estructura.) `design.md` con frontmatter `type: design` (más la clave `template:` copiada de la spec) y las secciones canónicas (cabecera + metadatos, `## Ficheros a crear o modificar`, `## Pasos` en el orden obligatorio, la matriz de trazabilidad — ver `design-contract.md` §7, §8, §10). Un `domains/<Entidad>.xml` por entidad del spec; un `views/<Fichero>.xml` por `<action-view>`; `menus.xml`. Cada `rules/R-*.md` referenciado desde `design.md` y viceversa.
...
- **f) Vistas** (`vistas.md` §3). Aplica **todas** las comprobaciones y detectores de `vistas.md` §3 sobre `views/*.xml`, `menus.xml` y los resúmenes estructurales del `design.md`: estructura de ficheros y PI `sv-*`, patrón `buttons-panel`, validación remota global, coherencia acción ↔ método, cierre `save` → `force-back`, forms modales de detalle y la **auditoría de layout (ASCII Layout)** — reconstruir el ASCII Layout de cada `<form>` desde los `colSpan`/`colOffset` reales, pasarle el «Checklist de maquetación» de `k-vistas/forms.md` y comprobar que coincide con los ASCII Layout declarados en el `design.md`. Cada incumplimiento es un fallo a reportar (los de `buttons-panel` son **bloqueantes**).
...
- **j) Coherencia diseño ↔ spec.** La comprobación depende de si la entidad/pantalla es nueva o existente (`design-contract.md` §1.3):
  - **Entidad/pantalla nueva** (sin marcador de existente en su `entity-*.md`/`screen-*.md`): los campos de cada `domains/<Entidad>.xml` coinciden con su `entity-*.md` (mismos nombres, mismos enums); las columnas/paneles de `views/*.xml` coinciden con su `screen-*.md`; los `<menuitem>` coinciden con los menús del spec.
  - **Entidad/pantalla existente** (marcada como existente — fila `Modificar`): el verificador **MUST** leer (nunca escribir) el fichero real de `src/main/...` que la fila referencia, y comprobar tres cosas: (1) todo campo/enum del **delta** del spec está en el XML del diseño; (2) todo elemento del **fichero real base** está en el XML del diseño, salvo los de "Eliminaciones declaradas" — conservar lo preexistente es **obligación**, no fallo; (3) ningún elemento del XML está fuera de base ∪ delta. Reportar como fallo cualquier elemento preexistente perdido sin declarar o cualquier añadido sin origen en el spec.
...
- **l) Acciones coherentes con el árbol real.** Cada fila `Modificar` de la tabla referencia un fichero que **existe** en el árbol real, y cada fila `Crear` uno que **no existe**. Una fila `Modificar` sin base real o una `Crear` que colisiona con un fichero existente es un fallo (la corrección es cambiar la Acción y aplicar `design-contract.md` §1.3).
