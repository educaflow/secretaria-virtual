...
- **h) Tests E2E** (`design/tests.md`, ver `tests-e2e.md`). Si el spec tiene escenarios, `tests.md` **MUST** existir y cada escenario del spec aparece como `Origen ESC` en al menos un test; cada `Verifica` y `Pantalla principal` referencia algo que existe. Si el spec no tiene escenarios, no se exige `tests.md`.
