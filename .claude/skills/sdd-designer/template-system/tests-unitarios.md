...
Como parte del diseño, **el subagente `test-unitarios`** escribe `design/unit-test-desc.md`: la **descripción** (no el código) de los tests unitarios JUnit 5 + Mockito necesarios para las clases Java que el diseño planifica. `/sdd-implementer` genera el código de los tests a partir de esta descripción.
...
- **MUST NOT** describir tests para:
  ...
  - Reglas que viven **solo en la capa cliente/XML** (`U-` de vistas: `showIf`/`readonlyIf`/`<action-validate>`/`<action-condition>` de cliente) → no son testables con JUnit; **MUST** listarlas como **excluidas** (van como E2E en `tests.md`).
...
- **MUST**: cada clase Java con lógica del diseño aparece en `unit-test-desc.md` (las omitidas, declaradas «sin lógica testable» con su razón).
- **MUST**: cada regla **server-side** (`V-`/`R-`/`CC-`) del diseño está cubierta por **≥1 test** (con su rama de fallo y, donde aplique, la rama OK).
- **MUST**: las reglas **solo-cliente** (`U-`) se listan como **excluidas** (E2E en `tests.md`), no se testean aquí.
- Cada test declara en `Verifica` la regla que ejerce (o `—` si es un método sin regla asociada, p.ej. un helper de cálculo puro).

## 5. Plantilla de `unit-test-desc.md`
...
Descripción de los tests unitarios (JUnit 5 + Mockito) por clase y método para el diseño. **Solo descripción, sin código**: `/sdd-implementer` genera el código a partir de aquí. Las reglas que viven solo en la capa cliente/XML (`U-`) no se testean aquí (van como E2E en `tests.md`).
...
- Reglas solo-cliente excluidas (E2E en tests.md): <lista de `U-`>.
```
...
El subagente **MUST NOT** devolver `ESCRITO: unit-test-desc.md` si queda algún punto sin cumplir. **LIMIT**: máximo 3 iteraciones de autocorrección.

---

## 7. Verificación de coherencia con el diseño (post-generación)

Tras escribir `unit-test-desc.md`, el skill lanza un bucle aparte con dos subagentes: **`verificador-test-unitarios`** (busca incoherencias entre `unit-test-desc.md` y el diseño) y **`corrector-test-unitarios`** (las corrige). Esta sección define **qué cuenta como incoherencia** — es la referencia del verificador. La **fuente de verdad** es el diseño (`design.md`): si un test no cuadra con el diseño, se corrige el **test**, nunca el diseño.

### 7.1 Comprobaciones del `verificador-test-unitarios`

- **Clase existe:** cada `## Clase: <FQN>` de `unit-test-desc.md` **MUST** corresponder a una clase Java que el diseño **crea o modifica** (en el inventario de `design.md`). Una clase descrita que el diseño no define → incoherencia `BLOCKING`.
...
- **Sin invención:** **MUST NOT** haber clases, métodos ni reglas en `unit-test-desc.md` que no estén en el diseño. Cualquier elemento inventado → `BLOCKING`.
...
- Aplica **en sitio** sobre `design/unit-test-desc.md` cada incoherencia reportada (eliminar/reasignar la clase/método/regla inexistente, corregir la cobertura, mover la `U-` a excluidas…), ajustándose a la plantilla §5.
